...
func (g *Group) UnmarshalXML(decoder *xml.Decoder, start xml.StartElement) error {
	...
	for {
		...
		case xml.StartElement:
			...
			case "rect":
				elementStruct = &Rect{group: g, Fill: g.Fill}
			case "circle":
				elementStruct = &Circle{group: g, Fill: g.Fill}
			case "path":
...
// GetPaths returns the paths within a group.
func (g *Group) GetPaths() []Path {
	paths := []Path{}
	for _, gn := range g.Elements {
		switch gn.(type) {
		case *Group:
			paths = append(paths, gn.(*Group).GetPaths()...)
		case *Path:
...
// GetRects returns the rects within a group.
func (g *Group) GetRects() []Rect {
	rects := []Rect{}
	for _, gn := range g.Elements {
		switch gn.(type) {
		case *Group:
			rects = append(rects, gn.(*Group).GetRects()...)
		case *Rect:
			rects = append(rects, *gn.(*Rect))
		}
	}
	return rects
}

// GetCircles returns the circles within a group.
func (g *Group) GetCircles() []Circle {
  	circles := []Circle{}
	for _, gn := range g.Elements {
		switch gn.(type) {
		case *Group:
			circles = append(circles, gn.(*Group).GetCircles()...)
		case *Circle:
			circles = append(circles, *gn.(*Circle))
		}
	}
	return circles
}
